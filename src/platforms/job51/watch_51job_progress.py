...
def render_social(snapshot: dict, manifest_total: int, raw_total: int, clean_total: int, cursor: dict | None = None) -> str:
    ...
    manual_verification_resolved = snapshot.get("manual_verification_resolved")
    manual_verification_result_known = manual_verification_resolved is not None
    elapsed = None
    eta = None
    stale_hint = ""
    ...
    if manual_verification_active or manual_verification_pause_count > 0:
        resolved_flag = bool(manual_verification_resolved) if manual_verification_result_known else True
        resolved_label = "已完成" if resolved_flag else "未完成"
        manual_state = "等待中" if manual_verification_active else ("最近已恢复" if resolved_flag else "最近未完成")
        ...
        if manual_verification_pause_count > 0 and manual_verification_result_known:
            lines.append(f"上次结果   {resolved_label}")
