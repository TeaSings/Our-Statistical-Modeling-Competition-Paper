...
注意：整理后的 GitHub 归档版本只保留脚本、配置和说明；全量 RAW、clean CSV、manifest、浏览器运行态已经移动到 `_local_archive_not_for_github/`。复现或续跑前，请先恢复需要的输入文件，或让脚本重新生成。
...
这个入口现在会默认写入并续跑 `_with_publish` 这一套 `raw / cursor / progress / manifest` 文件，不再回到已经清理掉的旧 social 快照。

可以直接复制这一行：

```powershell
python src/platforms/job51/run_sequential_social_crawl.py --transport browser --browser-min-interval 0.6 --browser-max-retries 4 --browser-speed-profile balanced --manual-verify --manual-verify-wait 120 --workers 12 --plan-prefetch-areas 4 --page-size 50 --specific-only
```
...
这个顺序调度器现在不会在爬取过程中自动清洗；要生成 clean CSV，请在你想要的时点手动执行：

```powershell
python src/clean_jobs.py --input data/raw/51job/records/51job_social_jobs_raw_with_publish.jsonl --output data/processed/51job/51job_social_jobs_clean_with_publish.csv
```
...
- 如果滑块或 `405` 触发后你没有在等待时间内完成人工验证，顺序调度器会把当前 batch 标成 `paused` 并保留页级 checkpoint；重新运行同一条命令即可续跑，不会再不断重建 client 和弹新空白窗口
...
- 遇到验证或页面卡住时，watcher 会提示“进度快照多久未更新”，并显示当前是否在等待共享浏览器的人工校验恢复，以及上一次校验是否真的完成
...
- 抓取和清洗现在彻底分离，顺序调度器运行时不再自动重刷 clean CSV
...
如果你想显式调节顺序调度器自己的 I/O 开销，还可以加：

- `--progress-write-interval 2`
- `--cursor-write-interval 2`
- `--plan-prefetch-areas 4`

旧的 `--refresh-clean*` 参数现在仅为兼容旧命令而保留，传了也会被忽略。
