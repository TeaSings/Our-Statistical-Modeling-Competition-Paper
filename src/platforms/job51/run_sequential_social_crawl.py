...
from platforms.job51.browser_search_client import ManualVerificationPendingError  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Deprecated and ignored by the crawler; use this path when you run src/clean_jobs.py manually after crawling",
    )
    parser.add_argument(
        "--refresh-clean",
        action="store_true",
        help="Deprecated and ignored; the crawler no longer refreshes clean CSVs while running",
    )
    parser.add_argument(
        "--refresh-clean-every-batches",
        type=int,
        default=DEFAULT_REFRESH_CLEAN_EVERY_BATCHES,
        help="Deprecated and ignored; kept only for backward-compatible CLI parsing",
    )
    parser.add_argument(
...
def manual_clean_command(args: argparse.Namespace) -> str:
    command = (
        "python src/clean_jobs.py "
        f"--input {args.output_raw} "
        f"--output {args.clean_output}"
    )
    if args.keep_empty_jd:
        command += " --allow-empty-jd"
    return command
...
def main() -> None:
    ...
    if args.refresh_clean:
        print(
            "Ignoring deprecated --refresh-clean flags. "
            "The sequential crawler now writes RAW/checkpoints only; "
            f"run manual clean later with: {manual_clean_command(args)}",
            flush=True,
        )
    else:
        print(
            "Automatic clean refresh is disabled for the sequential crawler. "
            f"Manual clean command: {manual_clean_command(args)}",
            flush=True,
        )
            ...
            while True:
                try:
                    ...
                    if isinstance(exc, ManualVerificationPendingError):
                        paused_batch = dict(cursor.get("current_batch") or current_batch)
                        paused_batch.update(
                            {
                                "status": "paused",
                                "last_error": str(exc),
                        ...
                        cursor["status"] = "paused"
                        cursor["current_batch"] = paused_batch
                        write_cursor_throttled(cursor_path, cursor, throttle=cursor_writer, force=True)
                        print(
                            "Paused because manual verification was not completed in time. "
                            "Finish the slider or switch network, then rerun the same command to continue "
                            "from the saved checkpoint without rebuilding extra browser windows.",
                            flush=True,
                        )
                        return
