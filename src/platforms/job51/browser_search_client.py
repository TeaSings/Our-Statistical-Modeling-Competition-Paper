...
class ManualVerificationCoordinator:
    def __init__(self) -> None:
        ...
        self._last_result_resolved = True
    ...
    def _snapshot_unlocked(self) -> dict[str, Any]:
        return {
            ...
            "resolved": self._last_result_resolved,
        }
    ...
    def begin(
        ...
        *,
        ...
        with self._condition:
            ...
            self._last_result_resolved = False
    ...
    def finish(
        ...
        *,
        ...
        with self._condition:
            ...
            self._last_result_resolved = bool(resolved)
            payload = self._snapshot_unlocked()
            self._owner = ""
            self._search_url = ""
            self._wait_seconds = 0
    ...
    def wait_for_resume(self) -> bool:
        with self._condition:
            while self._active:
                self._condition.wait(timeout=0.5)
            return self._last_result_resolved
...
class BrowserSearchClient:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cdp_url:
            ...
            shared_page = self._context.pages[0] if self._context.pages else None
            self._page = shared_page
            self._owns_page = False
            if self._page is None and self.reuse_existing_page_on_cdp:
                self._page = self._context.new_page()
                self._owns_page = True
            if self._page is not None and self.reuse_existing_page_on_cdp:
                self._prime()
    ...
    def _ensure_page(self) -> Any:
        if getattr(self, "_page", None) is None:
            if self._attached_via_cdp and self._context.pages:
    ...
    def _reprime(self) -> None:
        if self._attached_via_cdp and not self._owns_page:
    ...
    def _wait_for_manual_verification(self, search_url: str) -> bool:
        ...
        if coordinator is not None:
            ...
            if not is_owner:
                return coordinator.wait_for_resume()
        else:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                if self._attached_via_cdp:
                    ...
                    if blocked:
                        if self._wait_for_manual_verification(search_url):
                            continue
                        raise ManualVerificationPendingError(
                            f"manual verification was not completed in time for {self.worker_label}"
                        )
            ...
            except ManualVerificationPendingError:
                raise
            except Exception as exc:
                last_error = repr(exc)
