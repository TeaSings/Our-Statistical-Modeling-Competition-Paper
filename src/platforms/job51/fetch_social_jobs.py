...
from platforms.job51.browser_search_client import (  # noqa: E402
    BrowserSearchClient,
    ManualVerificationPendingError,
)
...
class ProgressState:
    ...
    manual_verification_resolved: bool = True
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "manual_verification_resolved": self.manual_verification_resolved,
    ...
    if isinstance(client, BrowserSearchClient):
        ...
        def handle_browser_status(event: str, payload: dict[str, Any]) -> None:
            if event == "manual_verification_waiting":
                ...
                state.manual_verification_resolved = False
                ...
                state.manual_verification_resolved = resolved
                try:
                ...
                if resolved:
                    ...
                    state.status_note = "manual verification incomplete; rerun will resume from this page"
            else:
                return
            ...
            if browser_fetch_workers <= 1:
                for page_index, (partition, page_num) in enumerate(page_tasks[resume_start_index:], start=resume_start_index):
                    ...
                    try:
                        ...
                        if isinstance(exc, ManualVerificationPendingError):
                            state.status_note = "manual verification incomplete; rerun will resume from this page"
                        else:
                ...
                try:
                    remaining_pages = max(len(page_tasks) - resume_start_index, 0)
                    fetch_window_size = min(
                        remaining_pages,
                        max(browser_fetch_workers * 2, browser_fetch_workers + 2),
                    )
                    if fetch_window_size <= 0:
                        fetch_window_size = browser_fetch_workers

                    pending_futures: dict[Future, int] = {}
                    ready_results: dict[int, tuple[Partition, int, list[dict[str, Any]]]] = {}
                    next_submit_index = resume_start_index
                    next_emit_index = resume_start_index
                    failed_index: int | None = None
                    failed_exc: Exception | None = None

                    def submit_page(page_index: int) -> None:
                        partition, page_num = page_tasks[page_index]
                        ...
                        pending_futures[future] = page_index

                    def failure_status(exc: Exception) -> str:
                        if isinstance(exc, ManualVerificationPendingError):
                            return "manual verification incomplete; rerun will resume from this page"
                        return "page fetch failed; rerun will resume from this page"

                    def emit_ready_results(*, limit_index: int | None = None) -> None:
                        nonlocal next_emit_index
                        while next_emit_index in ready_results and (
                            limit_index is None or next_emit_index < limit_index
                        ):
                            partition, page_num, items = ready_results.pop(next_emit_index)
                            emitted = write_page_items(
                                raw_file=raw_file,
                                items=items,
                                partition=partition,
                            ...
                            state.records_written += emitted
                            state.page_done = next_emit_index + 1
                            state.current_label = (
                    ...
                    while next_submit_index < len(page_tasks) and len(pending_futures) < fetch_window_size:
                        submit_page(next_submit_index)
                        next_submit_index += 1
                    ...
                    while pending_futures:
                        ...
                        for future in completed:
                            page_index = pending_futures.pop(future)
                            partition, page_num = page_tasks[page_index]
                            state.current_label = (
                                f"{partition.function_label} | {partition.job_area_label} | page {page_num}"
                            )
                            try:
                                ...
                                state.page_failures += 1
                                state.status_note = failure_status(exc)
                                if failed_index is None or page_index < failed_index:
                                    failed_index = page_index
                                    failed_exc = exc
                            else:
                                if failed_index is None or page_index < failed_index:
                                    ready_results[page_index] = (partition, page_num, items)

                        emit_ready_results(limit_index=failed_index)

                        if failed_index is not None:
                            for pending_future, pending_index in list(pending_futures.items()):
                                if pending_index > failed_index:
                                    pending_future.cancel()
                                    pending_futures.pop(pending_future, None)
                            for ready_index in [index for index in ready_results if index >= failed_index]:
                                ready_results.pop(ready_index, None)
                            if not any(index < failed_index for index in pending_futures.values()):
                                failed_partition, failed_page_num = page_tasks[failed_index]
                                state.current_label = (
                                    f"{failed_partition.function_label} | {failed_partition.job_area_label} | page {failed_page_num}"
                                )
                                state.status_note = failure_status(
                                    failed_exc or RuntimeError("page fetch failed")
                                )
