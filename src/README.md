...
注意：整理后的 GitHub 归档版本不再提交全量 raw/processed 数据。下面命令中的大 CSV、JSONL、HTML、manifest 默认需要先从 `_local_archive_not_for_github/` 恢复到原路径，或由脚本重新生成。
...
这个顺序调度器现在不会在爬取过程中自动清洗；手动清洗时单独运行：
...
- watcher 现在也会显示人工校验是否正在全局暂停、是哪一个 worker 触发的、上一次校验是否真的完成，以及最近一次恢复时间
