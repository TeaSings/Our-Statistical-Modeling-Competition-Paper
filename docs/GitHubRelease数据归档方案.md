...
| 附件 | 内容 |
| --- | --- |
| `raw-ncss-html-and-records.zip` | NCSS 原始 HTML、manifest 和 raw JSONL |
| `raw-51job-records.zip` | 51job 原始抓取记录和运行日志 |
| `processed-main-datasets.zip` | `data/processed/` 下的大体量主数据 |
| `analysis-static-full.zip` | 静态抽取全量中间表和完整 job-level 主表 |
| `checksums-sha256.txt` | 每个附件的 SHA256 校验值 |

每个附件保持小于 2 GiB；更大的目录先按来源或阶段拆包。
...
- 省赛前保持仓库和 Release 为私有；公开前再统一检查数据合规性和隐私风险。
